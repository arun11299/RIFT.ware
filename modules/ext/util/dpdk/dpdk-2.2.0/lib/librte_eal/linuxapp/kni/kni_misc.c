...
static int
kni_ioctl_clear_packet_counters(struct net *net,
                                unsigned int ioctl_num, unsigned long ioctl_param)
{
  ...
  list_for_each_entry_safe(dev, n, &knet->kni_list_head, list) {
    ...
    dev->v4_policy_fwd = 0;
    dev->v6_policy_fwd = 0;
    dev->forced_ndisc_sent = 0;
    dev->forced_arp_sent = 0;
    dev->bad_encap = 0;
 ...
 */
static int
kni_ioctl_get_packet_socket_info(struct net *net,
                                 unsigned int ioctl_num, unsigned long ioctl_param)
{
	int ret = -EINVAL;
...
static void *kni_seq_start(struct seq_file *seq, loff_t *pos)
{
...
static void *kni_seq_next(struct seq_file *seq, void *v, loff_t *pos)
{
  return NULL;
}

static void kni_seq_stop(struct seq_file *seq, void *v)
{
  struct net *net = seq_file_net(seq);
  struct kni_net *knet;
...
static int kni_seq_show(struct seq_file *seq, void *v)
{
  ...
  seq_puts(seq,
           "Index  Name      Net-namespace     rx  rx-as-tx   rx-as-tx-filter rx-as-tx-delv  rx-only  rx-filter  rx-delv  rx-no-skb tx-attempt tx tx-dropped tx-no-txq tx-no-allocq tx-enq-fail tx-deq-fail tx-noroute nl-tx-queue nl-tx-dequeue nl-rx-queued nl-rx-dequeued  v4-policy-fwd v6-policy-fwd arp-sent ndisc-sent  bad-policy-encap\n");
  knet = net_generic(net, kni_net_id);
  //down_read(&knet->kni_list_lock);
  list_for_each_entry_safe(kni, n, &knet->kni_list_head, list) {
    seq_printf(seq, "%-5d %-15s %s     %lu %llu %llu %llu %llu %llu %llu %lu %llu %lu %lu %llu %llu %llu %llu %llu %llu %llu  %llu %llu %llu %llu %llu %llu %llu\n",
               kni->net_dev->ifindex,
               kni->name, 
               kni->netns_name,
               ...
               kni->v4_policy_fwd, kni->v6_policy_fwd,
               kni->forced_arp_sent, kni->forced_ndisc_sent,
               kni->bad_encap);
  }
  //up_read(&knet->kni_list_lock);
  return 0;
...
static int kni_stats_show(struct seq_file *m, void *v)
{
  ...
  down_read(&knet->kni_list_lock);
  list_for_each_entry_safe(kni, n, &knet->kni_list_head, list) {
    seq_printf(m, "%-5d %-15s %s     %lu %llu %llu %llu %llu %llu %llu %lu %llu %lu %lu %llu %llu %llu %llu %llu %llu %llu  %llu %llu %llu %llu %llu %llu %llu\n",
               kni->net_dev->ifindex,
               kni->name, 
               kni->netns_name,
               kni->stats.rx_packets, kni->rx_treat_as_tx,
...
static const struct seq_operations kni_seq_ops = {
	.start = kni_seq_start,
	.next  = kni_seq_next,
	.stop  = kni_seq_stop,
	.show  = kni_seq_show,
	...
	unsigned long ioctl_param)
{
	...
	case _IOC_NR(RTE_KNI_IOCTL_RELEASE):
        ...
        case _IOC_NR(RTE_KNI_IOCTL_GET_PACKET_SOCKET_INFO):
          ret = kni_ioctl_get_packet_socket_info(net, ioctl_num, ioctl_param);
          break;
          case _IOC_NR(RTE_KNI_IOCTL_CLEAR_PACKET_COUNTERS):
            ret = kni_ioctl_clear_packet_counters(net, ioctl_num, ioctl_param);
            break;
#endif
	default:
