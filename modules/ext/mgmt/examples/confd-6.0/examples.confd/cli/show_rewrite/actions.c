...
static int do_show_path_rewrite(struct confd_user_info *uinfo,
...
int main(int argc, char **argv)
{
    ...
    strcpy(acb.actionpoint, "show_path_rewrite");
    acb.init = init_action;
    acb.command = do_show_path_rewrite;
...
{
    int i;
    char *new_path[] = {"arp","arpe","hwaddr"};
    char *unhide[] = {"foo"};
    char *tokens[] = {"permanent"};
    struct confd_rewrite_select select = { tokens, 1};
    struct confd_rewrite_select *selects[] = { &select };

    printf("do_show_path_rewrite called\n");

    printf("path as a flat string: %s\n", path);
    for (i = 0; i < argc; i++) {
        printf("token %d: %s\n", i, argv[i]);
    }

    if (i == 2 && strcmp(argv[1], "arp") == 0)
        if (confd_action_reply_rewrite2(uinfo, new_path, 3, unhide, 1,
                                        selects, 1) < 0)
