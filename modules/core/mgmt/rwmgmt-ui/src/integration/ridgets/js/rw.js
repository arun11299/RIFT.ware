...
rw.ui = {
  ...
  l10n : {
    vnf: {
      ...
      'iot_army': 'IOT Device Army',
      'iot_server': 'IOT App Server',
      'cag': 'Converged Access Gateway',
      'premise_gw': 'Premise Gateway'
    }
  }
};
...
rw.vcs = {
  ...
  allVms : function() {
    return _.flatten([ this.jpath('$.vm[*]'), this.jpath('$.collection[*].vm'), this.jpath('$.collection[*].collection[*].vm')], true);
  },
  ...
  vms: function(n) {
    ...
    switch (rw.vcs.nodeType(n)) {
      case 'rwcolony':
        return _.flatten([this.jpath('$.collection[*].vm[*]', n), this.jpath('$.vm[*]', n)]);
      case 'rwcluster':
  ...
  allClusters : function() {
    return _.flatten([this.jpath('$.collection[*].collection'),this.jpath('$.vm[*]')]);
  },

  allColonies: function() {
    return _.flatten([this.jpath('$.collection'),this.jpath('$.vm[*]')]);
  },
