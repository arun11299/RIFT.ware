(function(angular, window) {
  ...
  var controllerFn = function(radio, $rootScope, $scope, $timeout, slbFactory, vnfFactory) {
    var self = this;
    ...
    var listeners = [];
    $.when(slbFactory.attached(), vnfFactory.attached()).done(function() {

      listeners.push(appChannel.on("slb-update", function() {
        self.slbUpdateMetrics(slbFactory.slb.metrics);
      }, self));
    });
  ...
  _.extend(controllerFn.prototype, {
    created: function() {
      this.slbIncoming = [
  ...
  angular.module('slbalancer')
    .directive('slbalancerPanel', function() {
      return {
        restrict: 'AE',
        bindToController: true,
        controllerAs: 'slbalancerPanel',
        controller: controllerFn,
        templateUrl: '/modules/views/rw.slbalancer-dashboard_panel.tmpl.html',
        replace: true
      }
    });
