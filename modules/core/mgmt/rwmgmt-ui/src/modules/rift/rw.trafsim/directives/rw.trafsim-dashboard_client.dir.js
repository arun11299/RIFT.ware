(function(window, angular) {
  ...
  .directive('rwDashboardTrafsimClient', function(vnfFactory, portStateFactory, radio, vcsFactory, trafsimFactory) {
    return {
      restrict: 'AE',
      templateUrl:'/modules/views/rw.trafsim-dashboard_client.tmpl.html',
      replace: true,
      ...
      controllerAs: 'trafsimDashboardClient',
    ...
    function controllerFn($scope, $rootScope){
      ...
      self.gauges = [
        {label: 'Kcalls/s', rate: self.gbps, max: 210, color: 'hsla(212, 57%, 50%, 1)', colorLight: 'rgba(55,124,200,0.7)', paused: 'false'},
        {label: 'Kmsgs/s', rate: self.mpps, max: 600, color: 'hsla(260, 35%, 50%, 1)', colorLight: 'hsla(260, 35%, 50%, 0.7)', paused: 'false'}
      ];
      ...
      self.listeners.push(appChannel.on('trafsim-update', function() {
        ...
        self.gauges[0].rate = self.trafsimClient.call_rate / 1000;
        self.gauges[0].paused = !self.g.startedPerceived;
        self.gauges[1].rate = self.trafsimClient.tx_msg_per_sec / 1000;
