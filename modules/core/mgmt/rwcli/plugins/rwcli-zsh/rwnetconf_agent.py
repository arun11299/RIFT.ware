...
class CliChannel(object):
    ...
    def receive(self):
        ...
        Returns the decoded NetconfReq object.
        """
        # Read the message length first
        data = yield from self.stream_reader.read(4)
        if not data:
            logger.info('EOF Received')
            return None

        msglen = struct.unpack('I', data)[0]
        logger.debug('Message from CLI Len = {}'.format(msglen))

        # Get the protobuf message and decode it
        msg = yield from self.stream_reader.read(msglen)
        ncreq = rwuagent_pb2.NetconfReq()
        ncreq.ParseFromString(msg)
        return ncreq

    def respond(self, response_xml, ncreq):
        """Respond to the CLI with the encoded message.

        Arguments:
            response_xml - XML string response from Netconf server
            ncreq        - NCRequest protobuf message from CLI
        """
        ncrsp = rwuagent_pb2.NetconfRsp()
        ncrsp.operation = ncreq.operation
        ncrsp.xml_response.xml_blob = response_xml 
...
class NetconfChannel(object):
    ...
    def __init__(self, ncmgr, nc_agent):
    ...
    def connect(self, loop):
        ...
        while netconf_mgr is None and loop.is_running():
            try:
                ...
                #netconf_mgr._session.add_listener(self.session_event_listener)
                #self.has_candidate = CAP_CANDIDATE in self.netconf_mgr.server_capabilities
                nc_channel = NetconfChannel(netconf_mgr, self.nc_agent)
...
class NetconfAgent(object):
    ...
    def __init__(self, loop, read_fd, write_fd, host, port, username, passwd):
        ...
        self.nc_connector = NetconfConnector(self)
    ...
    def run(self):
        ...
        while self.loop.is_running():
            ncreq = yield from self.cli_channel.receive()
            if ncreq is None:
                # EOF received
                self.stop()
                break
            ...
            if not self.netconf_connected:
                ...
                self.cli_channel.respond('', ncreq)
                continue

            xml = self.prepare_xml(ncreq.xml_blob.xml_blob)
            response_xml = yield from self.netconf_channel.request(xml, ncreq)
            self.cli_channel.respond(response_xml, ncreq)
