...
rw_vx_modules_setup_libpeas(rw_vx_framework_t *rw_vxfp)
{
  ...
  char *rw_install_dir, *search_path, *plugin_loader_dir;
  rw_install_dir = getenv("RIFT_INSTALL");
  if (rw_install_dir) {
    search_path = RW_STRDUP_PRINTF("%s/usr/lib/girepository-1.0",
                                     rw_install_dir);
    plugin_loader_dir = RW_STRDUP_PRINTF("%s/usr/lib/libpeas-1.0/loaders",
                                     rw_install_dir);
  } else {
    search_path = RW_STRDUP_PRINTF("%s/usr/lib/girepository-1.0",
                                     INSTALLDIR);
  ...
  rw_vx_prepend_gi_search_path(search_path);
   ...
   */
  g_setenv("PEAS_PLUGIN_LOADERS_DIR", plugin_loader_dir, TRUE);
  g_setenv("PYTHONDONTWRITEBYTECODE", "yes", TRUE);

  
  RW_FREE(plugin_loader_dir);
  RW_FREE(search_path);
