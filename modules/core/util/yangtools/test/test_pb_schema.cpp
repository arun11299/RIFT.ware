...
static void get_merged_top_msgs(const rw_yang_pb_msgdesc_t* root1,
                                const rw_yang_pb_msgdesc_t* root2,
                                top_msg_list& mlist)
{
  for (unsigned i = 0; i < root1->child_msg_count; ++i) {

    auto child1 = root1->child_msgs[i];
    auto child2 = root2->child_msgs[i];

    if (child1 != child2) {
      ASSERT_STREQ (child1->yang_node_name, child2->yang_node_name);
      mlist.emplace_back(child1->yang_node_name);
    }
  }
}
                                

TEST(RwSchemaMerge, DISABLED_ImageComposite)
{
  YangModelNcx* model = YangModelNcx::create_model();
  YangModel::ptr_t p(model);
  ASSERT_TRUE(model);

  YangModule* comp = model->load_module("rw-composite");
  EXPECT_TRUE(comp);

  void *handle = dlopen ("librwcomposite_yang_parse_gen.so", RTLD_NOW|RTLD_LAZY|RTLD_NODELETE);
  RW_ASSERT(handle);

  const rw_yang_pb_schema_t* unified = nullptr;
  unsigned count = 0;

  for (auto mi = model->module_begin(); mi != model->module_end(); ++mi) {
    const char* mn = mi->get_name();
    if (std::string(mn) == "yuma-ncx" || std::string(mn) == "rw-composite") {
      continue;
    }
    auto sym_name = "rw_ypbc_" + YangModel::mangle_to_camel_case(mn) + "_g_schema";
    const rw_yang_pb_schema_t* schema =  (const rw_yang_pb_schema_t*)dlsym (handle, sym_name.c_str());
    if (!schema) {
      std::cout << "Failed to find symbol " << sym_name << std::endl;
      RW_ASSERT(schema);
    }

    std::cout << "Merging schema " << sym_name << std::endl;

    if (unified) {
      auto local = rw_schema_merge (nullptr, unified, schema);
      RW_ASSERT (local);
      unified = local;
    } else {
      unified = schema;
  ...
  std::cout << "Schemas loaded " << count << std::endl;

  ASSERT_TRUE (unified);

  auto sym_name = "rw_ypbc_" + YangModel::mangle_to_camel_case("rw-composite") + "_g_schema";
  const rw_yang_pb_schema_t* composite = (const rw_yang_pb_schema_t*)dlsym (handle, sym_name.c_str());
  RW_ASSERT (composite);

  ASSERT_EQ (composite->module_count, unified->module_count);
  verify_dynamic_schema(composite, unified);
  ...
  std::vector< std::pair<const rw_yang_pb_module_t*, const rw_yang_pb_module_t*> > merged_mods;
  for (size_t i = 0; i < unified->module_count; i++) {
    auto mod = unified->modules[i];
    if (NamespaceManager::get_global().ns_is_dynamic(mod->ns)){
      continue;
    }
    auto it = modules.find(mod->ns);
    ASSERT_NE(it, modules.end());
    if (it->second != mod) {
      if (strcmp(mod->module_name, "rw-composite")) {
        std::cout << "Merged module name " << mod->module_name << std::endl;
        merged_mods.push_back (std::make_pair(it->second, mod));
      }
    }
  }

  std::cout << "Merged modules " << merged_mods.size() << std::endl;

  for (auto& pair:merged_mods) {
    if (pair.first->data_module) {
      std::cout << "Compare data of " << pair.first->module_name << std::endl;
      verify_dynamic_merged_module_root(unified, pair.first->data_module, pair.second->data_module, false);

      top_msg_list ml;
      get_merged_top_msgs (pair.first->data_module, pair.second->data_module, ml);

      std::cout << "Merged msgs count " << ml.size() << std::endl;
      std::for_each (ml.begin(), ml.end(), [](const std::string& s) { std::cout << s << std::endl; });
      verify_merged_top_messages(unified, pair.first->data_module, pair.second->data_module, ml);
    }
    if (pair.first->rpci_module) {
      std::cout << "Compare rpci of " << pair.first->module_name << std::endl;
      verify_dynamic_merged_module_root(unified, pair.first->rpci_module, pair.second->rpci_module, false);

      top_msg_list ml;
      get_merged_top_msgs (pair.first->rpci_module, pair.second->rpci_module, ml);

      std::cout << "Merged msgs count " << ml.size() << std::endl;
      std::for_each (ml.begin(), ml.end(), [](const std::string& s) { std::cout << s << std::endl; });
      verify_merged_top_messages(unified, pair.first->rpci_module, pair.second->rpci_module, ml);
    }
    if (pair.first->rpco_module) {
      std::cout << "Compare rpco of " << pair.first->module_name << std::endl;
      verify_dynamic_merged_module_root(unified, pair.first->rpco_module, pair.second->rpco_module, false);

      top_msg_list ml;
      get_merged_top_msgs (pair.first->rpco_module, pair.second->rpco_module, ml);

      std::cout << "Merged msgs count " << ml.size() << std::endl;
      std::for_each (ml.begin(), ml.end(), [](const std::string& s) { std::cout << s << std::endl; });
      verify_merged_top_messages(unified, pair.first->rpco_module, pair.second->rpco_module, ml);
    }
  }
}
