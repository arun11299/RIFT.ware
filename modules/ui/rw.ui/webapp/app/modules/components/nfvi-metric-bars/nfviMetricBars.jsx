...
class nfviMetrics extends React.Component {
  ...
  render() {
    let html;
    let nfviMetrics = this.props.metrics.map(function(metric) {
      //Do not display network metrics
      if("outgoing" in metric || "incoming" in metric){
        return
      } else {
        return (<MetricChart utilization={metric.utilization} label={metric.label} total={metric.total} used={metric.used}/>)
      }
    });
    html = (
