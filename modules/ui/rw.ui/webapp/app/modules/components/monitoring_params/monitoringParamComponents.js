    ...
    switch(node["widget-type"]) {
      case 'GAUGE':
        return (
            ...
            mp={node["mp-id"]}>
                <div>{node.name}</div>
                <Components.Gauge value={node[valueProperty] || 0} min={numericConstraintsMinPresent ? node['numeric-constraints']['min-value'] : 0} max={numericConstraintsMaxPresent ? node['numeric-constraints']['max-value'] : 100} units={node['units'] || ''} />
            </div>);
        break;
      case 'TEXTBOX':
      case 'COUNTER':
